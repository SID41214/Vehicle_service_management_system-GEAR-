...
class ServiceCounterScreen extends StatefulWidget {
  ...
  @override
  State<ServiceCounterScreen> createState() => _ServiceCounterScreenState();
}

class _ServiceCounterScreenState extends State<ServiceCounterScreen> {
  ...
  createNew() async {
    String staffId = await getString(UserPreferences.USER_NUMBER);
     orderId = DateTime.now().millisecondsSinceEpoch.toString();
    HashMap<String, dynamic> data = HashMap();
    data['Id'] = orderId;
    data['createdBy'] = staffId;
    data['createdTime'] = DateTime.now().toString();
    data['Total'] = total;
    data['Discount'] = discount;
    data['GrandTotal'] = grandTotal;
    data['vehicleId'] = widget.vehicleModel.vehicleNo;
    data['CustomerId'] = widget.customerModel.id;
    data['CustomerName'] = widget.customerModel.name;
    data['Status'] = "PENDING";
    mRootReference.child("Services").child(orderId).update(data);
    mRootReference
        .child("Customer")
        .child(widget.customerModel.id)
        .child('Services')
        .child(widget.vehicleModel.vehicleNo)
        .set(orderId);
    mRootReference
        .child("Staff")
        .child(staffId)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        ):Container():(accessCode=='2'||accessCode=='3')?status=='PENDING'?
        Padding(
          padding: const EdgeInsets.all(8.0),
          child: MaterialButton(
            onPressed: () {
  ...
  Future<void> setProductToCart(int index) async {
    if (cart.any(
            (element) => element.serviceId == filterList[index].id)) {
      ...
      cart.add(
          CartModel(id: "Cart${cart.length+1}",
